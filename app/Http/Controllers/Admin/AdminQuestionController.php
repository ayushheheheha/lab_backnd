...
class AdminQuestionController extends Controller
{
    ...
    public function store(Request $request, int $quizId): JsonResponse
    {
        ...
        $stemTable = $this->parseStemTable($request);

        $created = DB::transaction(function () use ($validated, $request, $quizId, $options, $acceptableAnswers, $stemTable) {
            $stemImageUrl = null;
            ...
            $question = Question::query()->create([
                ...
                'marks' => $validated['type'] === 'comprehension' ? 0 : ($validated['marks'] ?? 1),
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        $acceptableAnswers = $this->parseAcceptableAnswers($request);
        $stemTable = $this->parseStemTable($request);
        $hasStemTablePayload = $request->has('stem_table_json') || $request->has('stem_table') || $request->boolean('remove_stem_table');
        $removeStemTable = $request->boolean('remove_stem_table');

        $updated = DB::transaction(function () use ($question, $validated, $request, $options, $acceptableAnswers, $stemTable, $hasStemTablePayload, $removeStemTable) {
            $payload = Arr::except($validated, ['options', 'acceptable_answers']);

            if ($hasStemTablePayload) {
                $payload['stem_table'] = $removeStemTable ? null : $stemTable;
            }
            ...
            $effectiveType = $payload['type'] ?? $question->type;

            if ($effectiveType !== 'numerical') {
                $payload['numerical_answer'] = null;
                $payload['numerical_tolerance'] = 0.01;
            }

            if ($effectiveType === 'comprehension') {
                $payload['marks'] = 0;
            }
    ...
    public function importJson(Request $request, int $quizId, QuestionImporter $importer): JsonResponse
    {
        ...
        $result = $importer->normalizeMany($validated['questions']);
    ...
    {
        $baseRules = [
            'type' => [$isCreate ? 'required' : 'sometimes', 'string', Rule::in(['mcq', 'multi_select', 'true_false', 'short_answer', 'numerical', 'comprehension'])],
            ...
            'stem_table_json' => ['nullable', 'string'],
            ...
            'remove_stem_table' => ['nullable', 'boolean'],
        ];
    ...
    private function parseStemTable(Request $request): ?array
    {
        $raw = $request->input('stem_table_json');
        if (is_string($raw) && $raw !== '') {
            $decoded = json_decode($raw, true);
            if (is_array($decoded)) {
                return $this->normalizeStemTable($decoded);
            }
        }

        $direct = $request->input('stem_table');
        if (is_array($direct)) {
            return $this->normalizeStemTable($direct);
        }

        return null;
    }

    private function normalizeStemTable(array $table): ?array
    {
